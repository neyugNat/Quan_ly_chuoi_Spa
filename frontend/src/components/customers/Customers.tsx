...
import { Search, Plus, Star, Phone, Calendar, Crown, Gift, XCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
function toTier(totalSpent: number) {
...
function toRating(visits: number) {
...
export function Customers() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const [customerRes, appointmentRes, invoiceRes] = await Promise.all([
        ...
        const invoiceRows = invoiceRes?.items || [];

        const visitByCustomer = new Map<number, number>();
        const lastVisitByCustomer = new Map<number, string>();
        const spentByCustomer = new Map<number, number>();

        appointmentRows.forEach((apt: any) => {
          const customerId = Number(apt?.customer_id || 0);
          if (!customerId) return;
          visitByCustomer.set(customerId, (visitByCustomer.get(customerId) || 0) + 1);

          const dateValue = String(apt?.start_time || '').trim();
          const current = lastVisitByCustomer.get(customerId);
          if (!current || dateValue > current) {
            lastVisitByCustomer.set(customerId, dateValue);
          }
        });
        ...
        invoiceRows.forEach((inv: any) => {
          ...
          if (!customerId) return;
          spentByCustomer.set(customerId, (spentByCustomer.get(customerId) || 0) + Number(inv?.total_amount || 0));
        });

        const mapped = customerRows.map((row: any) => {
          const id = Number(row?.id || 0);
          const visits = visitByCustomer.get(id) || 0;
          const totalSpent = spentByCustomer.get(id) || 0;
          return {
            id,
            name: row?.full_name || `Khách #${id}`,
            phone: row?.phone || '-',
            email: row?.email || '-',
            joined: formatDateDDMMYYYY(row?.created_at),
            joinedKey: String(row?.created_at || ''),
            visits,
            totalSpent,
            lastVisit: formatDateDDMMYYYY(lastVisitByCustomer.get(id)),
            tier: toTier(totalSpent),
            rating: toRating(visits),
          };
        });
  ...
  const newThisMonth = useMemo(() => {
    ...
    return customers.filter((c) => String(c?.joinedKey || '').startsWith(`${yyyy}-${mm}`)).length;
  }, [customers]);
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Khách mới tháng', value: newThisMonth, color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-100' },
        ].map((item) => (
            ...
            <div key={customer.id} className="bg-white rounded-2xl p-5 shadow-sm border border-[#e8eef8] hover:shadow-md hover:border-[#bfdbfe] transition-all">
              ...
              <div className="grid grid-cols-3 gap-2 mb-4 bg-[#f8faff] rounded-xl p-3">
                ...
                <div className="text-center border-x border-[#e8eef8]">
                  <div className="text-xs font-bold text-[#1d4ed8] truncate px-1">{formatMoneyVND(customer.totalSpent)}</div>
