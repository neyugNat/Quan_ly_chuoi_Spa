...
def ensure_seed_data() -> None:
    ...
    if admin is None:
        admin = User(username="admin", role="super_admin", is_active=True, branch_id=None)
    ...
    if manager is None:
        manager = User(username="manager", role="branch_manager", is_active=True, branch_id=b1.id)
        db.session.add(manager)
    manager.role = "branch_manager"
    manager.is_active = True
    manager.branch_id = b1.id
